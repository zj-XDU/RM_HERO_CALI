...
//底盘任务控制频率，尚未使用这个宏
#define CHASSIS_CONTROL_FREQUENCE 500.0f			//注意设置底盘任务的运行频率为500Hz，也即2ms
...
//底盘电机速度环PID
#define M3505_MOTOR_SPEED_PID_KP 1800.0f
...
typedef struct
{
  ...
  const struct bno055_euler_float_t *chassis_INS_angle;             //the point to the euler angle of gyro sensor.获取陀螺仪解算出的欧拉角指针
  ...
  uint8_t power_routin;				//底盘功率路径
  uint8_t power_routin0_en;		   //指示当前功率路径0是否使能
  uint8_t power_routin1_en;		   //指示当前功率路径1是否使能
