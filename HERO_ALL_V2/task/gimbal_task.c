...
void gimbal_task(void const *pvParameters)
{
	...
	//gimbal_control.gimbal_yaw_motor.max_relative_angle = 2.22503924;
	//gimbal_control.gimbal_yaw_motor.max_yaw_ecd = 1937;
	//gimbal_control.gimbal_yaw_motor.min_relative_angle = 1.49716532;
	
	//gimbal_control.gimbal_yaw_motor.gimbal_cali.min_yaw_ecd = 3341;
    ...
    while (1)
    {
		...
		osDelayUntil(&gimbal_task_pre_tick,2);
        //vTaskDelay(GIMBAL_CONTROL_TIME);
...
static void gimbal_init(gimbal_control_t *init)
{
	...
	//初始化yaw轴电机相对角度幅度的
	//init->gimbal_yaw_motor.max_relative_angle = 3.0f;//INIT_YAW_SET + 0.5;
	//init->gimbal_yaw_motor.min_relative_angle = -3.0f;//INIT_YAW_SET - 0.5;
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
   ...
   static fp32 bias_angle_rol;
    ...
    //now angle error
    //当前控制误差角度
    //bias_angle_rol = rad_format(gimbal_motor->absolute_angle_set - gimbal_motor->absolute_angle);
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
	...
	gimbal_motor->current_set = PID_calc(&gimbal_motor->gimbal_motor_absolute_gyro_pid, gimbal_motor->motor_gyro, gimbal_motor->motor_gyro_set);
    //控制值赋值
    
	//gimbal_motor->current_set = gimbal_PID_calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, gimbal_motor->absolute_angle_set, 0);
